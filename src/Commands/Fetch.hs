...
getRequest :: AccessorGD -> String -> IO (Either GError L.ByteString)
getRequest accessor postfix = do
  sendRequest accessor "GET" postfix

sendRequest :: AccessorGD -> String -> String -> IO (Either GError L.ByteString)
sendRequest accessor kind postfix = do
  manager <- newManager tlsManagerSettings
  initialRequest <- parseRequest $ accessor.endpoint ++ "/" ++ postfix
...
getDomains accessor = getRequest accessor accessor.endpoint >>= return . (\x -> maybeDecode x "000" "error extracting DomainSummary")


-- | Retrieve details for the specified Domain
getDomain :: AccessorGD -> Domain -> IO (Either GError DomainSummary)
getDomain accessor domain = getRequest accessor (accessor.endpoint ++ "/" ++ domain) >>= return . (\x -> maybeDecode x "000" "error extracting DomainSummary")


-- | Retrieve DNS Records for the specified Domain, optionally with the specified Type and/or Name
getDomainRecords :: AccessorGD -> Domain -> Maybe Type -> Maybe Name -> IO (Either GError [DNSRecord])
getDomainRecords accessor domain (Just t) (Just n) = getRequest accessor (domain ++ "/records/" ++ t ++ "/" ++ n) >>= return . (\x -> maybeDecodeL x "000" "error extracting DNSRecord")
getDomainRecords accessor domain (Just t) _ = getRequest accessor (domain ++ "/records/" ++ t) >>= return . (\x -> maybeDecodeL x "000" "error extracting DNSRecord")
getDomainRecords accessor domain _ (Just n) = getRequest accessor (domain ++ "/records/" ++ n) >>= return . (\x -> maybeDecodeL x "000" "error extracting DNSRecord")
getDomainRecords accessor domain _ _ = getRequest accessor (domain ++ "/records") >>= return . (\x -> maybeDecodeL x "000" "error etracting DNSRecord")


-- | Retrieve the legal agreement(s) required to purchase the specified TLD and add-ons
getLegalAgreements :: AccessorGD -> IO (Either GError [LegalAgreement])
getLegalAgreements accessor = getRequest accessor "agreements" >>= return . (\x -> maybeDecodeL x "000" "error extracting LegalAgreements")
